 ...
 *        satisfy the strict "> x" condition), so we discard it and
 *        everything to its left: low = mid + 1.
 *  - The loop continues while low < high. When it ends, low == high,
 *    and that value is exactly the upper bound.
 *
 * Note: This is nearly identical to Lower Bound - the ONLY difference
 * is the comparison operator (`>` here vs `>=` in lower bound).
 *
